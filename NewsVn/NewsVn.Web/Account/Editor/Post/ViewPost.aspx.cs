...
namespace NewsVn.Web.Account.Editor.Post
{
    public partial class ViewPost : BaseUI.SecuredPage
    {
        ...
        const string OrderBySK = "editor.post.sort.orderBy";
        const string OrderColumnSK = "editor.post.sort.orderColumn";
        const string OrderDirectionSK = "editor.post.sort.orderDirection";
        const string FilterExpressionSK = "editor.post.filter.expression";
        const string FilterModelSK = "editor.post.filter.model";
        ...
        protected void rptPostList_ItemDataBound(object sender, RepeaterItemEventArgs e)
        {
            ...
            {
                var ltrPostLink = e.Item.FindControl("ltrPostLink") as Literal;
                var postModel = e.Item.DataItem as PostModel;

                if (postModel.Approved)
                {
                    ltrPostLink.Text = clsCommon.getEllipsisText(postModel.Title, 30);
                }
                else
                {
                    ltrPostLink.Text = string.Format("<a href=\"{0}account/editor/post/editpost.aspx?pid={1}\""
                        +" class=\"{2}\" title=\"{3}\">{4}</a>", HostName, postModel.ID,
                        postModel.TitleCssClass.Trim(), postModel.Title, clsCommon.getEllipsisText(postModel.Title, 30));
                }
            }
        }
        ...
        private void LoadPostList(int pageIndex, int pageSize)
        {
            using (var ctx = new NewsVnContext(ApplicationManager.ConnectionString))
            {
                ...
                // Combine with this condition
                expression = expression.And(x => x.CreatedBy == User.Identity.Name);
                ...
                rptPostList.DataSource = ctx.PostRepo.Getter.getPagedList(posts, pageIndex, pageSize).Select(p => new PostModel
                {
                    ID = p.ID,
                    Title = p.Title,
