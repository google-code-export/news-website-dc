...
namespace NewsVn.Web
{
    ...
    {        
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!IsPostBack)
            {
                using (var ctx = new NewsVnContext(Utils.ApplicationManager.ConnectionString))
                {
                    ...
                    load_pletAdsDetail(AdsID, ctx);
                    bindBannerRight(ctx);
                }
            }
        }
        //load Ads details by ID
        private void load_pletAdsDetail(int AdsID, NewsVnContext ctx)
        {
        ...
        private void load_pletAdsRelated(Impl.Entity.AdPost inputAdPost, NewsVnContext ctx)
        {
